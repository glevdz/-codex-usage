...
function getProgressIcon(percent: number, colorCoding: boolean) {
  if (!colorCoding) return { source: Icon.Circle };
  if (percent >= 90) return { source: Icon.Circle, tintColor: Color.Red };
  if (percent >= 75) return { source: Icon.Circle, tintColor: Color.Orange };
  if (percent >= 50) return { source: Icon.Circle, tintColor: Color.Yellow };
...
export default function Command() {
  ...
  const loadUsage = useCallback(async () => {
    ...
    const prefs = getPreferenceValues<Preferences.SeeUsage>();
        ...
        <>
          ...
          <List.Section title="Weekly Limit">
            <List.Item
              ...
              accessories={[{ text: `${formatPercent(usage.weeklyPercentUsed)}` }]}
            />
            <List.Item
              title="Resets in"
